...
const Inventory = () => {
  ...
  const fetchInventoryData = async () => {
    ...
    try {
      ...
      // Buscar inventário primeiro
      const inventoryResponse = await fetch('https://tenisinventory-eqb9asgvheh0h5d5.brazilsouth-01.azurewebsites.net/api/inventory', {
        headers: { 
      ...
      console.log('📦 Resposta inventário:', inventoryResponse.status, inventoryResponse.statusText);

      if (!inventoryResponse.ok) {
        const errorText = await inventoryResponse.text();
        console.error('❌ Erro na requisição do inventário:', errorText);
        throw new Error(`Erro ao buscar inventário: ${inventoryResponse.status}`);
      }
      ...
      // Calcular stats localmente baseado nos dados do inventário
      let totalItems = 0;
      let totalValue = 0;
      ...
      if (inventoryData && Array.isArray(inventoryData)) {
        inventoryData.forEach(item => {
          ...
          totalValue += item.valorTotal || (item.quantidade * item.precoUnitario) || 0;
        });
      }

      console.log('🧮 Stats calculados localmente:', { totalItems, totalValue });

      // Tentar buscar stats das APIs separadas
      try {
        const countResponse = await fetch('https://tenisinventory-eqb9asgvheh0h5d5.brazilsouth-01.azurewebsites.net/api/inventory/count', {
          headers: { 
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          }
        ...
        console.log('🔢 Resposta count:', countResponse.status);

        if (countResponse.ok) {
          const countData = await countResponse.json();
          console.log('📈 Dados de contagem da API:', countData);
          
          // Usar dados da API se disponível, senão usar cálculo local
          if (countData && (countData.totalItems !== undefined || countData.count !== undefined)) {
            totalItems = countData.totalItems ?? countData.count ?? totalItems;
          }
        } else {
          console.warn('⚠️ API de contagem não disponível, usando cálculo local');
        }
      } catch (countError) {
        console.warn('⚠️ Erro ao buscar contagem, usando cálculo local:', countError);
      }

      try {
        const valueResponse = await fetch('https://tenisinventory-eqb9asgvheh0h5d5.brazilsouth-01.azurewebsites.net/api/inventory/value', {
          headers: { 
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          }
        ...
        console.log('💰 Resposta value:', valueResponse.status);

        if (valueResponse.ok) {
          const valueData = await valueResponse.json();
          console.log('💵 Dados de valor da API:', valueData);
          
          // Usar dados da API se disponível, senão usar cálculo local
          if (valueData && (valueData.totalValue !== undefined || valueData.value !== undefined)) {
            totalValue = valueData.totalValue ?? valueData.value ?? totalValue;
          }
        } else {
          console.warn('⚠️ API de valor não disponível, usando cálculo local');
        }
      } catch (valueError) {
        console.warn('⚠️ Erro ao buscar valor, usando cálculo local:', valueError);
      }
      ...
      console.error('❌ Erro geral ao buscar inventário:', error);
  ...
  const fetchTennisList = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error('Erro ao buscar tênis:', response.status, response.statusText);
      }
    } catch (error) {
  ...
  const handleAddToInventory = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        await fetchInventoryData(); // Aguardar a atualização
      } else {
        const errorData = await response.text();
        console.error('Erro ao adicionar:', response.status, errorData);
  ...
  const handleAdjustQuantity = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.text();
        console.error('Erro ao ajustar:', response.status, errorData);
  ...
  const handleRemoveFromInventory = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.text();
        console.error('Erro ao remover:', response.status, errorData);
